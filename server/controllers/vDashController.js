...
export const getNextEvent = async (req, res) => {
  try {
    ...
    const sql =
      "SELECT l.full_name, p.is_complete, em.event_id, em.start_time, em.end_time, em.event_description, em.event_location FROM login as l JOIN profile as p ON l.id = p.user_id LEFT JOIN volunteer_history as vh ON l.id = vh.volunteer_id LEFT JOIN eventManage as em ON vh.event_id = em.event_id WHERE l.id = ? AND (em.start_time > NOW() OR em.start_time IS NULL) ORDER BY em.start_time ASC LIMIT 1;";
